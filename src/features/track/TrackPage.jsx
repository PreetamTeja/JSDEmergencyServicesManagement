...
function fraction(d, now) {
  ...
  const total = d.eta_min || d.eta_to_pickup_min || 0
  if (d.eta_complete && total > 0) {
    const end = d.eta_complete * 1000
    const start = end - total * 60000
        ...
        <MapContainer center={center} zoom={13} zoomControl={false} className="h-full w-full">
          ...
          {pos && (
            <CircleMarker center={pos} radius={9}
              pathOptions={{ color: accent, fillColor: accent, fillOpacity: 1, weight: 3 }}>
              <Tooltip permanent direction="top" offset={[0, -10]}>{isFire ? '🚒' : '🚑'} {data.vehicle?.reg || 'Unit'}</Tooltip>
            </CircleMarker>
          )}
        </MapContainer>

        {/* Status card */}
        <div className="absolute left-3 right-3 bottom-3 sm:left-4 sm:bottom-4 sm:right-auto sm:w-80 bg-white rounded-xl shadow-card p-4">
